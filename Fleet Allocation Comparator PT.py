...
st.set_page_config(page_title="PT Files Comparator", layout="centered")
st.title("PT Files Comparator - Fleet Allocaiton")
...
if uploaded_file1 and uploaded_file2:
    try:
        file1 = pd.read_excel(uploaded_file1, dtype=str)
        ...
        file1 = file1[["Fleet Number", "Allocation date", "Allocation Status", "Depot Allocation", "PM Assign To"]]
        file2 = file2[["Fleet Number", "Allocation date", "Allocation Status", "Depot", "Assigned "]]
        ...
        for _, row in merged.iterrows():
            ...
            if pd.isna(row["Allocation date_file1"]):
                ...
                if row["Allocation Status_file1"] != row["Allocation Status_file2"]:
                    ...
                    StatusErrors += 1
                    error_IDs.add(num)
                if row["Depot Allocation"] != row["Depot"]:
                    problems.append(
                        f"⚠️Fleet Number {num} has different Depot "
                        f"({row['Depot Allocation']} vs {row['Depot']})"
                    )
                    DepotErrors += 1
                    error_IDs.add(num)
                if row["PM Assign To"] != row["Assigned "]:
                    problems.append(
                        f"⚠️Fleet Number {num} has different Assigned To "
                        f"({row['PM Assign To']} vs {row['Assigned ']})"
                    )
                    AssignedErrors += 1
                    error_IDs.add(num)
        ...
        for _, row in merged.iterrows():
            ...
            if pd.isna(row["Allocation date_file1"]):
                analytics_data.append({
                    "Fleet Number": num,
                    "Depot in SAP" : row["Depot Allocation"],
                    "Assigned To in SAP" : row["PM Assign To"],
                    "Type": "Missing Number",
            ...
            elif row["Depot Allocation"] != row["Depot"]:
                analytics_data.append({
                    "Fleet Number": num,
                    "Depot in SAP" : row["Depot Allocation"],
                    "Assigned To in SAP" : row["PM Assign To"],
                    "Type": "Depot Mismatch",
                    f"{file1_name}": row["Depot Allocation"],
                    f"{file2_name}": row["Depot"]
                })
            elif row["PM Assign To"] != row["Assigned "]:
                analytics_data.append({
                    "Fleet Number": num,
                    "Depot in SAP" : row["Depot Allocation"],
                    "Assigned To in SAP" : row["PM Assign To"],
                    "Type": "Assigned To Mismatch",
                    f"{file1_name}": row["PM Assign To"],
                    f"{file2_name}": row["Assigned "]
                })
        ...
        filename = st.text_input("File name for download", value="PT_fleet_allocation_comparison_result.xlsx")
